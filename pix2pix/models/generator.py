...
class Generator(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, conv_channels=64,
                 num_layers=4, conv_kernel=3, dropout=0.5):
        super(Generator, self).__init__()
        block_channels = [conv_channels * 2 ** power for power in range(num_layers)]
        self.input_layer = InputLayer(in_channels, conv_channels, conv_kernel)

        self.downsample_layers = [DownsampleBlock(num_channels, conv_kernel, dropout)
                                  for num_channels in block_channels]
        self.downsample_layers = nn.ModuleList(self.downsample_layers)

        self.upsample_layers = [UpsampleBlock(num_channels, conv_kernel, dropout)
...
def build_generator(params):
    return Generator(params.in_channels, params.out_channels, params.generator_channels,
                     params.generator_layers, params.generator_kernel, params.generator_dropout)
